...
def pkuseg_pos(string):
    ...
    for w in pos_list:
        word_s += w[0] + '  '
        pos_s += w[1] + '  '
    print('pos_tag time used: {} sec'.format((datetime.now() - start_time).total_seconds()))
    print(word_s)
    print(pos_s)
    print('\n\n')
...
def jieba_pos(string):
    print('Jieba的分词和词性标注：')
    start_time = datetime.now()
    pos_list = ([(x.word,x.flag) for x in psg.lcut(string)])
    word_s = ''
    pos_s = ''
    for w in pos_list:
        word_s += w[0] + '  '
...
def pyltp_pos(string):
    ...
    for p in postags:
        postag_list.append(p)
    word_s = ''
    pos_s = ''
    for w in word_list:
        word_s += w + '  '
    for p in postag_list:
        pos_s += p + '  '
    print('pos_tag time used: {} sec'.format((datetime.now() - start_time).total_seconds()))
    print(word_s)
    print(pos_s)
...
if __name__ == '__main__':
    string = '屠呦呦，女，中共党员，药学家，1930年12月30日生于浙江宁波。'
